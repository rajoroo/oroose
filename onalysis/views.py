...
def m15_analysis(symbol):
    ...
    df["m15_status"] = np.where(
        (df["ha_color"] == True) & (df["ha_color_1"] == True) & (df["ha_color_2"] == True) & (df["date_seq"] == 0),
        True,
        False,
    )

    df["ha_cross"] = np.where((df["ha_color"] == True) & (df["ha_color"].shift(-1) == False), True, False)
    ...
    df["m_val"] = np.where((df["m_min"] == df["inc_seq"]) | (df["m_max"] == df["inc_seq"]), True, False)
...
def hr_analysis(symbol):
    ...
    df = integrated_tool(df)
    df["stoch_cross_pos"] = np.where(
        (df["black"] > df["red"]) & (df["black"].shift(1) < df["red"].shift(1)), True, False
    )
    df["stoch_cross_neg"] = np.where(
        (df["black"] < df["red"]) & (df["black"].shift(1) > df["red"].shift(1)), True, False
    )
